...
const ProfileDrawer: React.FC<ProfileDrawerProps> = ({
  ...
  const joinedDate = useMemo(() => multiFormatDateString(otherUser.createdAt.toString()), [otherUser.createdAt]);

  const title = useMemo(() => data.name || otherUser.name, [data.name, otherUser.name]);

  const { members } = useActiveList();
  const isActive = useMemo(() => members.includes(otherUser?.email || ''), [members, otherUser]);
                    ...
                    <dl className="space-y-8 px-1 sm:space-y-6 sm:px-3">
                      {data.isGroup && (
                          <div>
                            ...
                            <dd className="mt-1 text-sm sm:col-span-2 text-gray-500">
